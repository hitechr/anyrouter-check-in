...
- 开启统计页面后，另在北京时间 9:10-21:00 每 10 分钟执行一次轻量用量采样（`STATS_ONLY` 模式，只查询用量，不签到、不通知）
- 你也可以随时手动触发签到

## 账户用量统计页面（可选）

统计页面完全使用 GitHub Actions、`stats-data` 分支和 GitHub Pages，不需要额外服务器或数据库。页面展示每个账号的当前余额、累计使用量、今日使用量、签到状态、最近 30 天趋势，以及基于 10 分钟采样的日/周/月用量热力图。
...
- 采样序列按月分片存放在 `stats-data` 分支的 `data/usage/YYYY-MM.json`，只含账号标识、别名、Provider 和累计用量
- 采样运行只提交数据分支、不重新部署 Pages；页面在 `*.github.io` 域名下直接从 `raw.githubusercontent.com` 读取分片（缓存约 5 分钟），数据延迟约为采样间隔加缓存时间
- 热力图单元格是相邻两次采样的累计用量差值，采样缺口（Actions 延迟、跨夜）显示为空格；10 分钟粒度会公开使用时段规律，介意请勿开启
